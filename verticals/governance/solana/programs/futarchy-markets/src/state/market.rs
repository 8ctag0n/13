...
// ============================================================================
// V3: Fully Blind Markets with FHE + ZK
// ============================================================================

pub const MARKET_V3_SEED: &[u8] = b"market_v3";

/// MarketV3 - Fully private prediction market
/// Seeds: ["market_v3", market_id]
///
/// Features:
/// - Encrypted pools (FHE) - nobody knows totals until settlement
/// - ZK proofs for pool updates
/// - Threshold decryption for settlement
/// - Pool commitments verified on-chain
#[derive(BorshSerialize, BorshDeserialize, Debug, Clone)]
pub struct MarketV3 {
    /// Market creator/authority
    pub authority: Pubkey,
    ...
    /// Commitment to encrypted pool state
    /// pool_commitment = Poseidon(pool_yes, pool_no, blinding)
    /// Prover knows plaintext pools, updates via ZK proof
    pub pool_commitment: [u8; 32],

    /// Hash of encrypted YES pool (FHE ciphertext stored off-chain)
    pub encrypted_pool_yes_hash: [u8; 32],

    /// Hash of encrypted NO pool (FHE ciphertext stored off-chain)
    pub encrypted_pool_no_hash: [u8; 32],

    /// Decrypted pools (revealed after threshold decryption)
...
impl MarketV3 {
    ...
    /// - resolution: 2 (Option<bool>)
    /// - pool_commitment: 32
    /// - encrypted_pool_yes_hash: 32
    /// - encrypted_pool_no_hash: 32
    /// - decrypted_pool_yes: 9 (Option<u64>)
    /// - decrypted_pool_no: 9 (Option<u64>)
    /// - threshold_pubkey: 32
    ...
    /// - bump: 1
    /// - created_at: 8
    /// - settled_at: 9 (Option<i64>)
    /// Total: 290 bytes
    pub const SPACE: usize = 290;
    ...
    pub fn new(
        ...
        Self {
            ...
            max_bet,
            resolution: None,
            // Initial pool commitment = Poseidon(0, 0, 0)
            // This allows first bet to use blinding=0 for empty pools
            pool_commitment: [
                0x0b, 0xc1, 0x88, 0xd2, 0x7d, 0xcc, 0xea, 0xdc,
                0x1d, 0xcf, 0xb6, 0xaf, 0x0a, 0x7a, 0xf0, 0x8f,
                0xe2, 0x86, 0x4e, 0xec, 0xec, 0x96, 0xc5, 0xae,
                0x7c, 0xee, 0x6d, 0xb3, 0x1b, 0xa5, 0x99, 0xaa,
            ],
            encrypted_pool_yes_hash: [0u8; 32],
            encrypted_pool_no_hash: [0u8; 32],
            decrypted_pool_yes: None,
            decrypted_pool_no: None,
            threshold_pubkey,
            threshold_required,
            threshold_shares,
            has_governance,
    ...
    /// Update pool commitment (after ZK-verified bet)
    pub fn update_pool_commitment(&mut self, new_commitment: [u8; 32]) {
        self.pool_commitment = new_commitment;
    }

    /// Update encrypted pool hashes (after FHE homomorphic addition)
    pub fn update_encrypted_pool_hashes(
        &mut self,
        yes_hash: [u8; 32],
        no_hash: [u8; 32],
    ) {
        self.encrypted_pool_yes_hash = yes_hash;
        self.encrypted_pool_no_hash = no_hash;
    }

    /// Settle the market with threshold-decrypted pools
    pub fn settle(
        &mut self,
        resolution: bool,
        ...
        self.resolution = Some(resolution);
        self.decrypted_pool_yes = Some(decrypted_pool_yes);
        self.decrypted_pool_no = Some(decrypted_pool_no);
        self.status = MarketStatus::Settled;
        self.settled_at = Some(current_time);
    }

    /// Get total pool (after settlement)
    pub fn total_pool(&self) -> Option<u64> {
        match (self.decrypted_pool_yes, self.decrypted_pool_no) {
            (Some(yes), Some(no)) => Some(yes.saturating_add(no)),
            _ => None,
        }
    }

    /// Get winning pool (after settlement)
    pub fn winning_pool(&self) -> Option<u64> {
        match self.resolution {
            Some(true) => self.decrypted_pool_yes,
            Some(false) => self.decrypted_pool_no,
            None => None,
        }
    }

    /// Get losing pool (after settlement)
    pub fn losing_pool(&self) -> Option<u64> {
        match self.resolution {
            Some(true) => self.decrypted_pool_no,
            Some(false) => self.decrypted_pool_yes,
            None => None,
        }
    }
}
