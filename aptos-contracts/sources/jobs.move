...
module zyberlink::jobs {
    ...
    public entry fun initialize(account: &signer, marketplace_addr: address) {
        ...
        move_to(account, JobsRegistry {
            ...
            escrows: table::new(),
            marketplace_addr,
            job_created_events: account::new_event_handle<JobCreatedEvent>(account),
            job_claimed_events: account::new_event_handle<JobClaimedEvent>(account),
            job_completed_events: account::new_event_handle<JobCompletedEvent>(account),
            fhe_result_submitted_events: account::new_event_handle<FheResultSubmittedEvent>(account),
            fhe_consensus_events: account::new_event_handle<FheConsensusEvent>(account),
        });
    }
    ...
    /// Check if FHE job has been finalized
    #[view]
    public fun is_fhe_finalized(registry_addr: address, job_id: u64): bool acquires JobsRegistry {
        assert!(exists<JobsRegistry>(registry_addr), E_NOT_INITIALIZED);
        let registry = borrow_global<JobsRegistry>(registry_addr);
        if (!table::contains(&registry.fhe_consensus, job_id)) {
            return false
        };
        let fhe_data = table::borrow(&registry.fhe_consensus, job_id);
        fhe_data.finalized
    }

    /// Get the consensus hash for a finalized FHE job
    #[view]
    public fun get_fhe_consensus_hash(registry_addr: address, job_id: u64): vector<u8> acquires JobsRegistry {
        assert!(exists<JobsRegistry>(registry_addr), E_NOT_INITIALIZED);
        let registry = borrow_global<JobsRegistry>(registry_addr);
        assert!(table::contains(&registry.fhe_consensus, job_id), E_NOT_FHE_JOB);
        let fhe_data = table::borrow(&registry.fhe_consensus, job_id);
        fhe_data.consensus_hash
    }
}
