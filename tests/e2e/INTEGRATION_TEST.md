...
This document explains how to run the complete end-to-end integration test that validates the entire ZyberLink marketplace workflow with a real prover node.
...
1. Deploy the program: `solana program deploy target/deploy/zyberlink.so`
...
cargo run --release --bin zyberlink-prover -- \
