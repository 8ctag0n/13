...
/// Program ID for cypherlink marketplace
...
/// Setup test environment with cypherlink program loaded
///
...
pub async fn create_test_fhe_job(
    ...
    let config: cypherlink_sdk::MarketplaceConfig =
    ...
    let create_job_ix = sdk.create_job_instruction(
        &creator.pubkey(),
        job_id,
        cypherlink_types::CircuitType::FheComputation(FheOperation::Add(10)),
        witness_commitment,
        65856,  // TFHE-rs FheUint8 ciphertext size
        (required_provers as u64) * 3_000_000,  // 0.003 SOL per prover
        300,  // 5min timeout
        Some(cypherlink_types::FheConsensusConfig {
            required_provers,
            consensus_threshold,
