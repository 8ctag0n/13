...
async fn test_finalize_fhe_job_consensus_reached() {
    ...
    // Setup and claim FHE job
    let (job_pda, escrow_pda, prover_pdas) = setup_fhe_job(
        &mut banks_client,
        &payer,
        &program_id,
        &config_pda,
    ...
    let prover3_authority = prover3_keypair.pubkey();
    let submit_result_instruction = MarketplaceInstruction::SubmitFheResult {
    ...
    let submit_result_ix = solana_program::instruction::Instruction {
        program_id,
        accounts: vec![
            solana_program::instruction::AccountMeta::new(prover3_authority, true),
            solana_program::instruction::AccountMeta::new(job_pda, false),
            solana_program::instruction::AccountMeta::new_readonly(
                solana_program::sysvar::clock::id(),
                false,
    ...
    assert_eq!(job.fhe_consensus_hash, Some(matching_hash));
}
...
async fn test_finalize_fhe_job_no_consensus() {
    ...
    // Add only prover1 (since only 1 result)
    finalize_accounts.push(solana_program::instruction::AccountMeta::new(
