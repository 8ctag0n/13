...
module zyberlink::marketplace {
    ...
    public entry fun initialize(
        ...
        move_to(account, MarketplaceConfig {
            ...
            provers: table::new(),
            prover_registered_events: event::new_event_handle<ProverRegisteredEvent>(account),
            prover_slashed_events: event::new_event_handle<ProverSlashedEvent>(account),
            config_updated_events: event::new_event_handle<ConfigUpdatedEvent>(account),
        });
    }
    ...
    public fun update_prover_on_success(
        ...
        if (total_jobs == 1) {
            ...
            prover.avg_completion_time_secs =
                ((prover.avg_completion_time_secs as u64 * 4 + completion_time_secs as u64) / 5) as u32;
        };
