...
use cypherlink::{instruction::MarketplaceInstruction, state::MarketplaceConfig, state::ProverAccount, state::JobAccount};
...
async fn test_create_job() {
    ...
    // Verify job account was created
    ...
    // Borsh may not use all allocated space - deserialize what's there
    let job: JobAccount = {
    ...
    // Verify escrow account exists and has correct balance
...
async fn test_create_job_invalid_price() {
    ...
    // Try to create a job with price = 0
    let job_creator = payer.pubkey();
    let job_id = 0u64;
    let job_id_bytes = job_id.to_le_bytes();
    let (job_pda, _) = Pubkey::find_program_address(
    ...
    let create_job_instruction = MarketplaceInstruction::CreateJob {
        ...
        witness_commitment: [1u8; 32],
        witness_size: 2048,
        price_lamports: 0, // Invalid price!
        timeout_seconds: 300,
    };
...
async fn test_create_multiple_jobs() {
    ...
    // Verify config has correct job count
